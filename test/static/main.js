var SocketMgr = {
	...
	SocketMgr.socket.on("buf",function(data) {
	    //console.log("heyy");
	    //Canvas.buf.enqueue(data);
	    Canvas.draw(data);
	});
	/*SocketMgr.socket.on("down", function(data) {
	    Canvas.draw(data);
	});*/
    }
};
...
var Canvas = {
    ...
    bindCanvasEvents: function() {
      	var fx = function(e) {
	    	Canvas.draw.isDrawing = true;
	    	SocketMgr.socket.emit("draw",{x:e.clientX,y:e.clientY});
      		Canvas.ctx.lineTo(e.clientX,e.clientY);
      		Canvas.ctx.stroke();
        };	
      	Canvas.canv.addEventListener("mousedown",function(e) {
	    	//SocketMgr.socket.emit("down",{x:e.clientX,y:e.clientY});
	    	//Canvas.draw.isDrawing = true;
	    	SocketMgr.socket.emit("draw",{x:e.clientX,y:e.clientY});
